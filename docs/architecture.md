...
| C7 | MCP server (`rmcp`, streamable HTTP) | Phase 3 | built: 9 read tools, eids + digests + latency on every response; compact items may carry parallel full records for the evidence store |
| C8 | Agent SOP (lead prompt + analyst instructions) | Phase 3 | SOP v4 (Phase 7): two-call triage (`freshness_watermark → build_evidence_bundle(focus_service)`) → hypotheses with eids → contradiction check (incl. `nearest_deploy.relation`) → correlational labelling with the deploy offset → three exits → verify → cited postmortem |
