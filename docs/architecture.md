...
| C7 | MCP server (`rmcp`, streamable HTTP) | Phase 3 | built: 11 tools (9 read + the causal check's `get_exemplar_request`, `replay_exemplar`), eids + digests + latency on every response; compact items may carry parallel full records for the evidence store |
| C8 | Agent SOP (lead prompt + analyst instructions) | Phase 3 | SOP v5 (Phase 8): two-call triage (`freshness_watermark → build_evidence_bundle(focus_service)`) → hypotheses with eids → contradiction check (incl. `nearest_deploy.relation`) → conditional fan-out (three analyst briefs in `agent/subagents/`, only when the bundle leaves a hypothesis unresolved) → **causal check** (`get_exemplar_request → replay_exemplar`; `separated` earns "caused", `not_separated` says which way) → three exits (ACT needs a separated replay when one was possible) → verify → cited postmortem with the replay's limit stated |
