...
| Model | **Config parameter** — any provider TrueForge supports; `MODEL_A`/`MODEL_B` env for the runner | Required by the benchmark and the generalization experiment (next sections). The engine is model-agnostic by construction: nothing in it knows which model is reading |
