...
declare -A PORT=([deployer]=8792 [rawtools]=8793)
...
case "${1:-status}" in
  start)  for n in deployer rawtools; do start_one "$n" || exit 1; done ;;
  stop)   for n in deployer rawtools; do p="$(pid_on "${PORT[$n]}")"; [ -n "$p" ] && kill "$p" && echo "$n: stopped ($p)" || echo "$n: not running"; done ;;
  status) for n in deployer rawtools; do p="$(pid_on "${PORT[$n]}")"; [ -n "$p" ] && echo "$n: up on :${PORT[$n]} (pid $p)" || echo "$n: down"; done ;;
