...
use spyglass_core::{VerifyCfg, Window};
...
pub fn judge_at(
    ...
    if r.post_requests < cfg.min_requests {
        return if elapsed_secs > cfg.timeout_secs {
            ("timeout", false, true)
        } else {
    ...
    if clean {
        ...
        return if n >= cfg.checks_required {
    ...
    let rising = st.checks > 0
        && st.consecutive_clean == 0
        && st
            .last_post_rate
            .is_some_and(|prev| r.post > prev + cfg.tolerance_abs);
...
/// Seconds since this investigation's last *counted* check of `service:deploy_id`,
/// or None if there was none. The MCP server uses it to wait out the rest of
/// `interval_secs` before running a check, so the pacing is the engine's --
/// the agent no longer needs a sandbox `sleep` between checks (Phase 10 F6a).
pub fn secs_since_last_check(
    engine: &Engine,
    investigation: &str,
    service: &str,
    deploy_id: &str,
    ...
    let now = Utc::now();
    engine.with_investigation(investigation, |inv| {
        inv.verifications
            .get(&format!("{service}:{deploy_id}"))
            .and_then(|s| s.last_check_at)
...
pub fn verify_recovery(
    ...
    let (action, reverted, safe, baseline_w, incident_w, post_w, counts) = {
        ...
        let incident_from = reverted
            .as_ref()
            .map(|d| d.ts)
            .unwrap_or(action.ts - Duration::seconds(vc.incident_lookback_secs));
        let incident_w = Window {
            from: incident_from,
            to: action.ts,
        };
        let baseline_w = Window {
            from: incident_from - Duration::seconds(vc.baseline_secs),
            to: incident_from,
        };
        let post_from = (safe - Duration::seconds(vc.window_secs)).max(action.ts);
        let post_w = Window {
            from: post_from,
            to: safe.max(post_from),
        };
    ...
    let (cps, _) = crate::tools::detect_changepoints(
        ...
        &ChangepointArgs {
            ...
            route: None,
            window: WindowArg {
                from: Some(fmt_ts(incident_w.from)),
                to: Some(fmt_ts(safe)),
            },
            baseline: WindowArg {
                from: Some(fmt_ts(incident_w.from)),
                to: Some(fmt_ts(incident_w.to)),
            },
    ...
    let (status, clean, escalate, st, closed_now) =
        engine.with_investigation(investigation, |inv| {
    ...
    let next = match status {
        ...
        "clean" => format!("clean ({}/{}); call verify_recovery again (the engine waits out the {} s interval before checking)", st.consecutive_clean, vc.checks_required, vc.interval_secs),
        "insufficient_data" => format!("only {} request(s) since the action; call again (the engine waits out the {} s interval)", rates.post_requests, vc.interval_secs),
        "too_soon" => format!("not counted: {} s since the last check, checks must be {} s apart; call again", since_last, vc.interval_secs),
        "not_recovered" => format!("post rate {} above tolerance {}; call again (the engine waits out the {} s interval; escalates at {} s after the action)", pct(rates.post), pct(tol), vc.interval_secs, vc.timeout_secs),
...
mod tests {
    ...
    fn two_consecutive_clean_checks_close_the_incident() {
        ...
        assert_eq!(
            judge(&r(0.0, 0.2, 0.0, 100), &st, 30, &c),
            ("clean", true, false)
        );
        let st2 = VerifyState {
            checks: 1,
            consecutive_clean: 1,
            last_post_rate: Some(0.0),
            ..Default::default()
        };
        assert_eq!(
            judge(&r(0.0, 0.2, 0.01, 100), &st2, 45, &c),
            ("recovered", true, false)
        );
    }
    ...
    fn a_dirty_check_resets_the_streak_and_waits() {
        ...
        assert_eq!(
            judge(&r(0.0, 0.2, 0.08, 100), &st, 45, &c),
            ("not_recovered", false, false)
        );
    }
    ...
    fn no_better_than_the_incident_escalates_immediately() {
        let c = cfg();
        assert_eq!(
            judge(&r(0.0, 0.2, 0.2, 100), &VerifyState::default(), 20, &c),
            ("worsening", false, true)
        );
        assert_eq!(
            judge(&r(0.0, 0.2, 0.35, 100), &VerifyState::default(), 20, &c),
            ("worsening", false, true)
        );
    }
    ...
    fn rising_between_checks_while_dirty_escalates() {
        ...
        let st = VerifyState {
            checks: 1,
            consecutive_clean: 0,
            last_post_rate: Some(0.05),
            ..Default::default()
        };
        assert_eq!(
            judge(&r(0.0, 0.2, 0.09, 100), &st, 40, &c),
            ("worsening", false, true)
        );
    }
    ...
    fn the_timeout_escalates_instead_of_retrying_forever() {
        let c = cfg();
        assert_eq!(
            judge(&r(0.0, 0.2, 0.08, 100), &VerifyState::default(), 301, &c),
            ("timeout", false, true)
        );
        assert_eq!(
            judge(&r(0.0, 0.2, 0.0, 3), &VerifyState::default(), 301, &c),
            ("timeout", false, true)
        );
    }
    ...
    fn too_few_requests_is_not_a_verdict() {
        let c = cfg();
        assert_eq!(
            judge(&r(0.0, 0.2, 0.0, 5), &VerifyState::default(), 10, &c),
            ("insufficient_data", false, false)
        );
    }
    ...
    fn tolerance_scales_with_a_noisy_baseline() {
        ...
        assert_eq!(
            judge(&r(0.04, 0.3, 0.055, 100), &VerifyState::default(), 10, &c).0,
            "clean"
        );
        assert_eq!(
            judge(&r(0.04, 0.3, 0.07, 100), &VerifyState::default(), 10, &c).0,
            "not_recovered"
        );
    }
    ...
    fn a_check_inside_the_interval_is_not_counted() {
        ...
        assert_eq!(
            judge_at(&r(0.0, 0.2, 0.0, 500), &st, 30, 1, &c),
            ("too_soon", false, false)
        );
        assert_eq!(
            judge_at(&r(0.0, 0.2, 0.0, 500), &st, 45, 15, &c),
            ("recovered", true, false)
        );
        // the first check is never too soon
        assert_eq!(
            judge_at(&r(0.0, 0.2, 0.0, 500), &VerifyState::default(), 5, 0, &c),
            ("clean", true, false)
        );
    }
    ...
    fn closed_and_escalated_are_terminal() {
        let c = cfg();
        let closed = VerifyState {
            closed: true,
            ..Default::default()
        };
        assert_eq!(
            judge(&r(0.0, 0.2, 0.5, 100), &closed, 10, &c),
            ("recovered", true, false)
        );
        let esc = VerifyState {
            escalated: true,
            escalation_reason: Some("worsening".into()),
            ..Default::default()
        };
        assert_eq!(
            judge(&r(0.0, 0.2, 0.0, 100), &esc, 10, &c),
