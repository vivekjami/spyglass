...
- novel_templates — WHAT is new: log templates that are new or bursting in the window vs the baseline, ranked. This is where the root cause almost always is. Read novelty_reason, dominant_level, has_stack, first_seen_in_window, and instances.
- detect_changepoints — WHEN it changed: error-rate / error-count / latency / traffic changepoints per service, route and instance, ordered earliest first, each with `at`, magnitude_x and nearest_deploy (deploy_id, offset_secs, relation). The earliest changepoint is the likeliest origin; `nearest_deploy.relation` answers "did the change follow a deploy?" directly.
...
- get_evidence — dereference an eid: the full record plus raw exemplars. Use it on the top novel ERROR hit to read the stack.
...
1. TRIAGE: freshness_watermark → novel_templates → detect_changepoints → deploy_events. Four calls. Add error_delta by service only if the picture is still unclear.
2. HYPOTHESES: 1–3 candidates, each with the eids that motivate it.
3. CHECK: get_evidence on the top novel ERROR hit. Then the CONTRADICTION CHECK before advancing anything: does the template's first_seen_in_window predate the deploy? Does the earliest changepoint precede the deploy (`nearest_deploy.relation`)? Is there a change event closer in time? Is a different service the origin (earliest first_seen / earliest changepoint, has_stack)? Record rejected hypotheses with eids.
4. LABEL: deploy-then-error timing is CORRELATIONAL. Say so, with the offset in seconds from the changepoint's nearest_deploy. Do not write "caused" without a controlled comparison (causal replay is not available in this version — state that).
