...
7. VERIFY after an executed action — the engine judges and paces, you ask: verify_recovery(service, deploy_id = the rollback's journal_entry.deploy_id). Read `status` and `next`:
   - `clean`, `not_recovered` or `insufficient_data` → call verify_recovery again, immediately: the engine waits out the 15 s interval itself before checking, so do not sleep, poll the watermark, or ask the time in between.
